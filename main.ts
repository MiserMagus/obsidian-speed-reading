...
class SpeedReadModal extends Modal {
  ...
  private static readonly DISPLAY_CHARS = 20; // Use more space for long words
  private static readonly PIVOT_POS = 10; // Centered in 20-char display
  ...
  onOpen() {
    ...
    this.displayEl.addEventListener("wheel", (e: WheelEvent) => {
      e.preventDefault();
      if (this.isRunning) {
        this.stop();
      }
    ...
    this.displayEl.addEventListener("keydown", (e: KeyboardEvent) => {
      const now = Date.now();
      if (now - lastNavTime < 100) return; // debounce: 100ms
      lastNavTime = now;
      if (this.isRunning) this.stop(); // Always pause RSVP before manual navigation
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      editorCallback: (editor: Editor) => {
        ...
        this.currentModal = new SpeedReadModal(
          this.app,
          selectedText,
